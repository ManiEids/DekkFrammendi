...
interface DekkjaKortProps {
  dekk: Dekk;
  onAddToSamanburdur?: () => void;
  isSelected?: boolean;
}

export default function DekkjaKort({ dekk, onAddToSamanburdur, isSelected }: DekkjaKortProps) {
    ...
    <div className={`border rounded-lg overflow-hidden bg-white shadow-md transition ${isSelected ? 'ring-2 ring-blue-500' : 'hover:shadow-lg'}`}>
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          {dekk.framleiddAf && <p className="text-sm text-gray-600 mb-3">Framleiðandi: {dekk.framleiddAf}</p>}
          
          <div className="mt-auto">
            <p className="text-lg font-bold text-blue-600">{formatPrice(dekk.verd)}</p>
            <p className={`text-sm ${dekk.fjoldiALager && dekk.fjoldiALager > 0 ? 'text-green-600' : 'text-red-600'}`}>
              {getStockStatus(dekk.birgdaStada, dekk.fjoldiALager)}
            </p>
          </div>
        </div>
        ...
        {onAddToSamanburdur && (
          <div className="px-4 pb-4">
            <button 
              ...
              className={`w-full py-2 px-4 rounded text-sm ${isSelected ? 
